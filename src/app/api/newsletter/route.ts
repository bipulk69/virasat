...
import { 
  ...
  adminNewsletterNotificationTemplate 
} from '@/utils/email';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const emailResult = await sendEmailWithAdminNotification(
      newsletterEmailTemplate(email),
      adminNewsletterNotificationTemplate(email)
    );
    ...
    // For example: await db.collection('subscribers').insertOne({ email, subscribedAt: new Date() });
